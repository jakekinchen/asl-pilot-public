#!/usr/bin/env python3
"""Compute per-class FAR10/FAR5 accept thresholds for recognizer-simccw48.onnx.

Scores the signer-disjoint TEST split of the run10 SimCC-w48 feature cache
(/Users/kelly/ws1-recog-extract-ws/cache-simccw48) with the trained run10
checkpoint (the exact weights exported to
web/public/practice/recognizer-simccw48.onnx) and derives, per class, the
softmax threshold at a fixed false-accept rate over NEGATIVE clips (other-class
test clips scored against this class). Semantics replicate
asl-pilot-annotator/tools/detector0-annotator/verification.py
(recall_at_far_details): tau_w = quantile(neg_scores_w, 1 - FAR).

Template: web/scripts/compute-rtmpose-thresholds.py (the sidecar machinery
added at commit 22ca99b5); only the model/cache/sanity references changed.

Outputs:
  web/public/practice/recognizer-simccw48-thresholds.json  (sidecar consumed by
      SignPracticeApp.tsx at model-load time)
  /tmp/simccw48-accept-rule-simulation.json  (old vs new accept rule, 8 demo words)

Run (CPU, ~2 min):
  /Users/kelly/Developer/asl-pilot/.venv/bin/python \
      web/scripts/compute-simccw48-thresholds.py
"""
...
from train_recognizer import load  # noqa: E402  (the EXACT run10 featurizer)
...
# crater this toward chance; the session-962 receipt measured 0.9209 with test
# top-1 0.5428 (independently re-scored locally, abs diff 3.6e-05).
EXPECTED_MEAN_RECALL_FAR10 = (0.88, 0.96)
REFERENCE_TOP1 = 0.5428
...
def main() -> None:
    ...
    model = SeqTransformer(
        feat=int(ck["feat"]), n_classes=len(labels), d_model=int(ck["d_model"]),
        n_layers=int(ck["n_layers"]), n_heads=int(ck["n_heads"]), dropout=float(ck["dropout"]),
    )
    ...
    above_floor = sum(1 for v in far_thresholds["far10"].values() if v > OLD_FLOOR)
    print(f"mean recall@FAR10 {mean10:.4f} | mean recall@FAR5 {mean5:.4f} | "
          f"FAR10 thresholds above the {OLD_FLOOR} floor: {above_floor}/{len(labels)}")
    lo, hi = EXPECTED_MEAN_RECALL_FAR10
    if not (lo <= mean10 <= hi):
        sys.exit(f"SANITY FAIL: mean recall@FAR10 {mean10:.4f} outside [{lo}, {hi}] — not shipping thresholds")

    sidecar = {
        "model": "recognizer-simccw48.onnx",
        "source": "ws1-recog-extract-ws cache-simccw48 signer-disjoint test split",
    ...
    out = WEB / "public/practice/recognizer-simccw48-thresholds.json"
    out.write_text(json.dumps(sidecar, indent=1) + "\n")
    print(f"wrote {out}")

    sim = simulate(probs, yt, labels, far_thresholds["far10"])
    sim_out = Path("/tmp/simccw48-accept-rule-simulation.json")
    sim_out.write_text(json.dumps({
        "model": "recognizer-simccw48.onnx",
        "eval": "cache-simccw48 test split, 8 demo words as prompts",
        "rules": {
