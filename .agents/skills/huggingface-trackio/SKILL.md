...
description: Use in ASL Pilot when a future authorized training or evaluation run needs Trackio metric logging, dashboarding, alerts, or CLI metric retrieval alongside the existing JSON receipt/provenance chain. Do not replace repo receipts or audits with Trackio alone.
---

# Trackio - Experiment Tracking for ML Training

ASL Pilot local policy: Trackio can supplement training telemetry, but
`training-provenance.json`, `docs/validation/*.json`, session logs, and
existing audit scripts remain required evidence.
