---
name: hf-cli
description: "Use in ASL Pilot only for explicit Hugging Face Hub CLI (`hf`) work: auth, cache, repos, datasets, jobs, approved artifact transfer, or Hub inspection. Do not trigger for generic ML work, and do not upload raw learner video or traces unless the user explicitly approves the exact target."
---

ASL Pilot local policy: `GOAL.md`, the active prompt, and
`.agents/skills/asl-pilot-ml-receipt/SKILL.md` override this upstream skill.
