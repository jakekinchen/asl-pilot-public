...
const schemaVersion = "asl-pilot-wikimedia-commons-negative-challenge-review-packet/v1";
const requiredTypes = [
  "empty_camera",
  "no_hands_visible",
  "low_light",
...
function usage() {
  console.log(`Usage:
  node scripts/export_wikimedia_commons_negative_challenge_review_packet.mjs [--candidates docs/research/wikimedia-commons-negative-challenge-candidates.json] [--downloads docs/research/wikimedia-commons-negative-challenge-downloads.json] [--output docs/review/wikimedia-commons-negative-challenge-review-packet.json]

Exports a visual-review packet for Wikimedia Commons negative challenge
candidates. The packet is input to review only; it is not final source approval
or model evidence.
`);
}
...
function buildPacket(args) {
  ...
  return {
    ...
    generated_by: {
      ...
      script: {
        path: "scripts/export_wikimedia_commons_negative_challenge_review_packet.mjs",
        sha256: sha256File(path.join(root, "scripts", "export_wikimedia_commons_negative_challenge_review_packet.mjs")),
      },
    },
    candidates: {
...
try {
  ...
  console.error(`Wikimedia Commons review packet export failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 2;
}
